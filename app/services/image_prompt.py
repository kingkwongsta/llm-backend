...
from models.model_image_prompt import ImagePrompt
...
def generate_image_prompt(llm_model, input):
    llm = LLMFactory(llm_model)
    messages = [
        {
            "role": "system",
            "content": "you are an expert prompt engineer specializing in text to image prompts",
        },
        {
            "role": "user",
            "content": f"create a high quality text to image prompt about {input}",
        },
    ]
    ...
    assert isinstance(completion, ImagePrompt)
